...
| `AttributeError: module 'queue' has no attribute 'LifoQueue'` al correr `recognition-app/src/main.py` directamente | `recognition-app/src/` tenía su propio paquete llamado `queue/` (la cola local de reintentos) — al ejecutar un script dentro de `src/`, Python agrega esa carpeta al principio de `sys.path`, así que `import queue` (lo hace `urllib3`, una dependencia de `requests`) encontraba el paquete del proyecto en vez del módulo real de la librería estándar | Renombrado a `recognition-app/src/local_queue/` — nunca nombrar un paquete propio igual que un módulo de la librería estándar de Python |
